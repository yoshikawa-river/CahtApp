...
func testMessageToOneRoomUsingRoom(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := MessageSlice{&local}
	if err = local.L.LoadRoom(ctx, tx, false, (*[]*Message)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Room == nil {
		t.Error("struct should have been eager loaded")
...
func testMessageToOneSetOpRoomUsingRoom(t *testing.T) {
	var err error
...
var (
	messageDBTypes = map[string]string{`ID`: `bigint`, `Content`: `varchar`, `UserID`: `bigint`, `RoomID`: `bigint`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_              = bytes.MinRead
)
