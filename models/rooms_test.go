...
func testRoomToManyMessages(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoomSlice{&a}
	if err = a.L.LoadMessages(ctx, tx, false, (*[]*Room)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoomToManyAddOpMessages(t *testing.T) {
	var err error
...
var (
	roomDBTypes = map[string]string{`ID`: `bigint`, `Name`: `varchar`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_           = bytes.MinRead
)
